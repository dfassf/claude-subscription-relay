...
- OAuth 토큰 설정 (macOS Keychain 자동 추출 / 직접 입력 / 나중에 /login)
- Telegram 봇 설정 (선택)
- Docker 빌드 및 실행

## OAuth 토큰 자동 갱신

`CLAUDE_CODE_REFRESH_TOKEN`이 설정되어 있으면 토큰이 만료되기 전에 자동으로 갱신됩니다.
- 백그라운드에서 1분마다 만료 체크, 만료 5분 전에 선제적 갱신
- 갱신된 토큰은 `tokens/oauth.json`에 저장 (컨테이너 재시작 대비)
- API 호출 중 인증 오류 발생 시 1회 자동 재시도

## 인증 (수동)

```bash
# 방법 1: OAuth 로그인 (원격 서버용)
curl -X POST http://서버주소:8000/login -H "X-API-Key: <your-key>"
...
| 변수 | 기본값 | 설명 |
|------|--------|------|
| `CLAUDE_CODE_OAUTH_TOKEN` | (없음) | OAuth access token |
| `CLAUDE_CODE_REFRESH_TOKEN` | (없음) | OAuth refresh token (자동 갱신용) |
| `API_KEY` | (없음) | API 인증 키 |
